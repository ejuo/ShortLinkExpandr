 ...
 var lib = {   
    expand: function(links) {
      ...
      if (ae_links.length > 0) {
        this.aeExpand(ae_links);
      }
      return longs;
    },
    
    store: function(short_link, link) {      
      window.localStorage.setItem('sl-' + short_link, link);
    },
    ...
    aeExpand: function(short_links) {
		...
		opera.extension.postMessage({q:tmp.join('&'),w:"http://4.expndr.appspot.com/?" + tmp.join('&')});
		return;
    },
    
    find: function(short_link) {      
      var long_link = window.localStorage.getItem('sl-' + short_link);
...
(function() {
  ...
  function expandLinks() {
	...
	expand_map = {}, expansion_queue = [];
    links = collectionToArray(d.getElementsByTagName("a"));
    for (i=0, len=links.length; i<len; ++i) {
      ...
      if (match != null && match.hasOwnProperty('2') && supported.hasOwnProperty(match[2].toLowerCase())) {
        ...
        link.expanding = true;
        cached_long_link = window.localStorage.getItem(href);
    ...
    while (expansion_queue.length > 0) {
      ...
      if (expanded == 0) {
        ...
        var i, j,
		longs = lib.expand(expansion_request),
        indices;
	...
	body.addEventListener('DOMNodeInserted', expandLinks, false);
	
	function step4(event){
		var  i, j, len, indices,
		message1  = event.data.q;
		message2  = event.data.w;
		shorts = message1.split("\n");
		longs = message2.split("\n");
		for (i=0, len = longs.length; i<len; ++i) {
			lib.store(shorts[i], longs[i]);
		}
        // Update each short url.
		for (i=shorts.length-1; i>=0; --i) {
			indices = expand_map[shorts[i]];
			for (j=indices.length-1; j>=0; --j) {           
				addLongURL(links[indices[j]], longs[i]);
			}
		}
    }
	opera.extension.onmessage = step4;
})();
}, false);
