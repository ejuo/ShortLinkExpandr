var shortLinkExpander = function () {
	return {
		init : function () {
			if(gBrowser) gBrowser.addEventListener("DOMContentLoaded", function () {
					shortLinkExpander.run();
			}, false);
		},
		...
		run : function () {
			...
			store: function(short_link, link) {      
			  window.content.localStorage.setItem('sl-' + short_link, link);
			},
			...
			aeExpand: function(short_links) {
			  ...
			  for (i=0, len = tmp.length-1; i<len; ++i) {
			...
			find: function(short_link) {       
			  var long_link = window.content.localStorage.getItem('sl-' + short_link);
			...
			(function() {
				...
				for (i=0, len=links.length; i<len; ++i) {
				  ...
				  if (match != null && match.hasOwnProperty('2') && supported.hasOwnProperty(match[2].toLowerCase())) {
					...
					cached_long_link = window.content.localStorage.getItem(href);
				...
				window.addEventListener("load", tryExpandLinks, false); 

			})();
		}
	};
}();
window.addEventListener("load", function() { shortLinkExpander.init(); }, false);